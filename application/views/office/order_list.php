								...
								<div id="kt_app_content_container" class="app-container container-xxl">
									...
									<div class="card">
										...
										<div class="card-body py-4">
											<!--begin::Table-->
											<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_search_workshop">
												<thead>
												...
												<tbody class="text-gray-600 fw-semibold">
													...
													<tr>
														...
														<td><?=ucfirst(get_user_detail($key['user_id'],'name'))?><br><?=get_user_detail($key['user_id'], 'email')?></td>
														<td><?=get_user_detail($key['user_id'], 'phone_no')?></td>
														<td><span class="badge badge-success badge"><?=$key['status']?></span></td>
														<td><b><?=$key['remark']?></b></td>
														<td class="text-end">

															<? if($key['status'] == "PRINT"){ ?>
															<a href="#" class="btn btn-success btn-flex btn-center btn-sm complete-order" data-init="<?=$key['id']?>">Complete Order</a>
															<? } else { ?>
															<? if($key['status'] == 'CONFIRM'){ ?>
															<a href="#" class="btn btn-secondary btn-flex btn-center btn-sm print-order" data-init="<?=$key['id']?>">Print</a>
															<? } else { ?>
															<a href="#" class="btn btn-info btn-flex btn-center btn-sm confirm-order" data-init="<?=$key['id']?>">Confirm Order</a>
															<? } ?>
															<a href="#" class="btn btn-danger btn-flex btn-center btn-sm reject-order" data-init="<?=$key['id']?>">Reject</a>
															<? } ?>
														</td>
													</tr>
													<? } ?>
